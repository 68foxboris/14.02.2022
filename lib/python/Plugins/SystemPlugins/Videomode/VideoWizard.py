...
class VideoWizard(WizardLanguage, Rc):
	...
	def listModes(self):
		...
		print "[Videomode] modeslist:", list
		return list

	def modeSelectionMade(self, index):
		print "[Videomode] modeSelectionMade:", index
		self.mode = index
		self.modeSelect(index)

	def modeSelectionMoved(self):
		print "[Videomode] mode selection moved:", self.selection
		self.modeSelect(self.selection)

	def modeSelect(self, mode):
		ratesList = self.listRates(mode)
		print "[Videomode] ratesList:", ratesList
	...
	def listRates(self, querymode=None):
		if querymode is None:
			querymode = self.mode
		list = []
		print "[Videomode] modes for port", self.port, "and mode", querymode
		for mode in self.hw.getModeList(self.port):
			print "[Videomode] mode:", mode
			if mode[0] == querymode:
				for rate in mode[1]:
					if self.port == "DVI-PC":
	...
	def rateSelectionMade(self, index):
		print "[Videomode] rateSelectionMade:", index
		self.rate = index
		self.rateSelect(index)

	def rateSelectionMoved(self):
		print "[Videomode] rate selection moved:", self.selection
	...
	def showTestCard(self, selection=None):
		...
		print "[Videomode] set config.misc.showtestcard to", {'yes': True, 'no': False}[selection]
		if selection == "yes":
